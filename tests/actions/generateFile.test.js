...
describe('generateFile action', () => {
  ...
  it('should generate file with tags and manual ignores', async () => {
    ...
    download.mockResolvedValue('/dummy/.gitignore');
    fs.appendFileSync.mockImplementation(() => {});

    await generateFile('/dummy');

    expect(os).toHaveBeenCalled();
    expect(project).toHaveBeenCalledWith('/dummy');
    expect(download).toHaveBeenCalledWith({
      directory: '/dummy',
      tags: ['linux', 'node', 'react'],
    });

    expect(fs.appendFileSync).toHaveBeenCalledWith('/dummy/.gitignore', '# custom\r\nignored_dir');

    expect(console.info).toHaveBeenCalledWith('[gign] generated at /dummy/.gitignore');
  ...
  it('should handle nothing detected', async () => {
    ...
    // It should still log generated because download is called with just OS tag
    expect(console.info).toHaveBeenCalledWith('[gign] nothing detected');
  });
  ...
  it('should handle errors gracefully', async () => {
    ...
    expect(console.error).toHaveBeenCalledWith('[gign] OS Error');
  });
});
